...
if __name__ == '__main__':
    rclpy.init_node('ground_truth_tf_publisher')
    tf_listener = tf2.TransformListener()
    
    rate = rclpy.Rate(100)
    received_odometry_to_map = False
    while not rclpy.is_shutdown() and not received_odometry_to_map:
        try:
            (p_map_odom1, q_map_odom1) = tf_listener.lookupTransform('map', 'husky_1/odom', rclpy.Time(0))
    ...
    if received_odometry_to_map:
        real_odom_sub1 = rclpy.Subscriber("/husky_1/odometry/ground_truth", Odometry, odom1_callback)
